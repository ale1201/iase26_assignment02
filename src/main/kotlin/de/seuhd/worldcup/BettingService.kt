...
object BettingService {
    ...
    fun evaluateBonus(matches: List<Match>): Int {
        val matchesById = matches.associateBy { it.matchId }
        var total = 0
        for (bet in bets.values) {
            val match = matchesById[bet.matchId] ?: continue
            val home = match.homeScore ?: continue
            val away = match.awayScore ?: continue
            val actualOutcome = Prediction.outcomeOf(home, away)
            if (bet.prediction != actualOutcome) continue
            total += if (bet.predictedHomeScore == home && bet.predictedAwayScore == away) 3 else 1
        }
        return total
    }
